...
-- Create Welcome Tab
local WelcomeTab = Window:CreateTab("Welcome", nil) -- Title, Image



-- Display the current game name

local WelcomeSection = WelcomeTab:CreateSection("Welcome")

local displayText = player.DisplayName

-- Check if the player is one of the specified IDs
...
-- Display in the label on WelcomeTab
...
spawn(function()
    while true do
        TimeLabel:Set("Time: " .. os.date("%I:%M %p")) -- Updates the label with hours and minutes
        wait(1) -- Updates every second
    end
end)
...
-- Call the function to check the game initially
checkGame()



-- Assuming the label is already created
local LabelMessage = WelcomeTab:CreateLabel("Known Errors/Bugs: ".. Errors .. Date)

-- Function to change the label text to a loading message and then back to the normal message
...
-- Create a button in your UI library
local DiscordButton = WelcomeTab:CreateButton({
	...
	Callback = function()
		...
		if setclipboard then
			...
			print("Copied to clipboard: https://discord.gg/ZNfKFyuUEd") -- Confirmation in the console
...
local TargetTab = Window:CreateTab("👤 Target", nil) -- Title, Image
...
local Section = CharacterTab:CreateSection("General")

-- Helper function to apply Korblox leg
local function applyKorblox(character)
    local rightLeg = character:FindFirstChild("Right Leg")
    if rightLeg then
        rightLeg:Destroy()
        
        local korbloxRightLeg = Instance.new("MeshPart")
        korbloxRightLeg.Name = "Right Leg"
        korbloxRightLeg.Size = Vector3.new(1, 2, 1)
        korbloxRightLeg.MeshId = "http://www.roblox.com/asset/?id=4506945409"
        korbloxRightLeg.TextureID = "http://roblox.com/asset/?id=4506945426"
        korbloxRightLeg.Transparency = 0
        korbloxRightLeg.CanCollide = false
        korbloxRightLeg.Parent = character
    end
end

-- Helper function to restore default leg
local function restoreDefaultLeg(character)
    local rightLeg = character:FindFirstChild("Right Leg")
    if rightLeg then
        rightLeg:Destroy()
        
        local newLeg = Instance.new("Part")
        newLeg.Name = "Right Leg"
        newLeg.Size = Vector3.new(1, 2, 1)
...
-- Headless toggle
local HeadlessToggle = CharacterTab:CreateToggle({
...
local RefreshButton = CharacterTab:CreateButton({
   ...
   Callback = function()
      ...
      Rayfield:Notify({
         ...
         Content = "Your character will respawn with applied customizations",
         Duration = 2,
      })
   end,
})

-- Client-side reminder label
